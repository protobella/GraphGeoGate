...
this.mouse2D = new THREE.Vector3(0, 10000, 0.5);
  ...
  createHexSphere(iNumSubdiv) {
    ...
    for (var pataticas = 0; pataticas < iNumSubdiv; pataticas++) {
      ...
      this.g_HexPlanetData = new Array();
      ...
      for (var i = 0; i < aNeighborsInfo.length; i++) {
       ...
       function HexData() {
        ...
        this.pCurrentHexData = new HexData();
        this.pCurrentHexData.fHeight = random_length;
        this.pCurrentHexData.iCenterId = i;
        this.pCurrentHexData.vCenter = this.hexoplanetGeometry.vertices[i];
        // console.log(this.hexoplanetGeometry.faces.length)
        for (var j = 0; j < aNeighborsInfo[i].length; j++) {
          if (pataticas < iNumSubdiv - 1) {
            this.pCurrentHexData.aFaces.push(this.hexoplanetGeometry.faces.length);
            this.pCurrentHexData.aTopFaces.push(this.hexoplanetGeometry.faces.length);
  ...
  // lbl_fcs.appendChild(valuelbl_fcs)
        ...
        this.g_HexPlanetData.push(this.pCurrentHexData);
      }
    }
    ...
    //Calculate neighbors
    //------------------
    for (var i = 0; i < this.g_HexPlanetData.length; i++) {
      var center = this.g_HexPlanetData[i].vCenter;
      var aClosests = new Array();
  
      for (var j = 0; j < this.g_HexPlanetData.length; j++) {
        var newEntry = new Object();
        newEntry.id = j;
        newEntry.distance = center.distanceToSquared(this.g_HexPlanetData[j].vCenter);
      ...
      for (var j = 0; j < this.g_HexPlanetData[i].aTopFaces.length; j++) {
        this.g_HexPlanetData[i].aNeighbors.push(aClosests[j + 1].id);
      }
  ...
  // }
   ...
   onDocumentMouseDown(event) {
    ...
    if (intersects.length > 0) {
      ...
      var hexID;
      for (var i = 0; i < this.g_HexPlanetData.length; i++) {
        for (var j = 0; j < this.g_HexPlanetData[i].aFaces.length; j++) {
          if (this.g_HexPlanetData[i].aFaces[j] == faceID) {
            hexID = i;
      ...
      //document.getElementById( "lbl_ngh" ).innerHTML = 'Neighbors: ' + 	this.this.g_HexPlanetData[hexID].aNeighbors;
  
      switch (event.button) {
        case 0:
          this.g_HexPlanetData[hexID].fHeight += 10;
          break;
        case 2:
          this.g_HexPlanetData[hexID].fHeight -= 10;
          break;
  ...
  //
   ...
   doSmooth() {
    ...
    for (var i = 0; i < this.g_HexPlanetData.length; i++) {
      ...
      for (var j = 0; j < this.g_HexPlanetData[i].aNeighbors.length; j++) {
        neighbordsHeight +=
          this.g_HexPlanetData[this.g_HexPlanetData[i].aNeighbors[j]].fHeight;
      }
      neighbordsHeight = neighbordsHeight / this.g_HexPlanetData[i].aNeighbors.length;
      newHeights.push((this.g_HexPlanetData[i].fHeight + neighbordsHeight) / 2.0);
    }
  
    // Assign the smooth heights
    for (var i = 0; i < this.g_HexPlanetData.length; i++) {
      this.g_HexPlanetData[i].fHeight = newHeights[i];
    }
   ...
   doRefreshGeometry() {
    ...
    for (var i = 0; i < this.g_HexPlanetData.length; i++) {
      ...
      for (var j = 0; j < this.g_HexPlanetData[i].aTopFaces.length; j++) {
        var currentFace =
        this.g_hexoplanetGeometry.faces[this.g_HexPlanetData[i].aTopFaces[j]];
  
        this.g_hexoplanetGeometry.vertices[currentFace.a].normalize();
        var escalar = this.g_HexPlanetData[i].fHeight;
