      ...
      function (text) {
        ...
        if (typeof metadata !== "undefined") {
          ...
          if (metadata.format !== "assimp2json") {
            ...
            onError(
              "THREE.AssimpJSONLoader: Unsupported assimp2json file format version."
            );
            return;
          }
        }
  ...
  parse: function (json, path) {
    ...
    function parseObject(json, node, meshes, materials) {
      ...
      for (i = 0; node.meshes && i < node.meshes.length; i++) {
        idx = node.meshes[i];
        obj.add(
          new THREE.Mesh(meshes[idx], materials[json.meshes[idx].materialindex])
        );
      }
